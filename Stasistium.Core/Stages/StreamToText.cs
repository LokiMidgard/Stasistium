...
namespace Stasistium.Stages
{
    ...
    {
        public StreamToText(StagePerformHandler<Stream, TInCache> input, Encoding? encoding, GeneratorContext context) : base(input, context)
        {
...
namespace Stasistium
{
    ...
    {
        public static StreamToText<T> ToText<T>(this StageBase<Stream, T> input, Encoding? encoding = null)
            where T : class
        {
            if (input is null)
                throw new ArgumentNullException(nameof(input));
            return new StreamToText<T>(input.DoIt, encoding, input.Context);
        }
    }
}
